...
#' Script LDA (linear discriminant analysis)
#' 
#' 22/07/22
...
lda = MASS::lda(formula = Customer_Segment ~ ., data = training_set)
training_set = as.data.frame(predict(lda, training_set))
training_set = training_set[c(5, 6, 1)]

colnames(training_set) <- c("x.LD1", "x.LD2", "Customer_Segment")

lda = MASS::lda(formula = Customer_Segment ~ ., data = test_set)
test_set = as.data.frame(predict(lda, test_set))
test_set = test_set[c(5, 6, 1)]

colnames(test_set) <- c("x.LD1", "x.LD2", "Customer_Segment")
